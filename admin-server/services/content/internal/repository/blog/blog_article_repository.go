...
func NewBlogArticleRepository(store *repository.Store) BlogArticleRepository {
	return &blogArticleRepository{
		articleModel: store.BlogArticleModel,
		articleTag:   store.BlogArticleTagModel,
		conn:         store.DB,
	}
}
...
func (r *blogArticleRepository) CreateWithTags(ctx context.Context, article *blogmodel.BlogArticle, tagIDs []uint64) error {
	...
	// 领域服务包在 store.Transact 里调用，失败直接返回 err 触发整个事务回滚即可。
