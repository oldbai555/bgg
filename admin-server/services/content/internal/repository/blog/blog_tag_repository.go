...
type BlogTagRepository interface {
	FindPage(ctx context.Context, page, pageSize int64, name string, status int64) ([]blogmodel.BlogTag, int64, error)
	FindByID(ctx context.Context, id uint64) (*blogmodel.BlogTag, error)
	// FindEnabledList 查询启用标签列表（用于下拉选项）
	FindEnabledList(ctx context.Context, limit int64) ([]blogmodel.BlogTag, error)
	Create(ctx context.Context, tag *blogmodel.BlogTag) error
	Update(ctx context.Context, tag *blogmodel.BlogTag) error
	Delete(ctx context.Context, id uint64) error
}

type blogTagRepository struct {
	model blogmodel.BlogTagModel
	conn  sqlx.SqlConn
}

func NewBlogTagRepository(store *repository.Store) BlogTagRepository {
	return &blogTagRepository{
		model: store.BlogTagModel,
		conn:  store.DB,
	}
}

func (r *blogTagRepository) FindByID(ctx context.Context, id uint64) (*blogmodel.BlogTag, error) {
...
func (r *blogTagRepository) Update(ctx context.Context, tag *blogmodel.BlogTag) error {
...
func (r *blogTagRepository) FindPage(ctx context.Context, page, pageSize int64, name string, status int64) ([]blogmodel.BlogTag, int64, error) {
	// 使用 squirrel 构建动态查询
	conditions := sq.And{
		sq.Eq{"deleted_at": 0},
	}
...
func (r *blogTagRepository) FindEnabledList(ctx context.Context, limit int64) ([]blogmodel.BlogTag, error) {
	if limit <= 0 {
		limit = 1000
	}
	conditions := sq.And{
	...
	// 查询所有字段，以匹配 blogmodel.BlogTag 结构体
