...
// BuildSources 根据配置初始化数据源，供 ServiceContext 调用。iam-rpc 自己的 config.Config
// 只有 Mysql.DSN + IamRedis 两块（不是 gateway 原来 DatabaseConf 的连接池细粒度配置），
// 和 sdk/chat/content 三个已拆分服务的 servicecontext.go 同一个精简程度。
func BuildSources(cfg config.Config) (*Repository, error) {
	if cfg.Mysql.DSN == "" {
		return nil, errors.New("iam-rpc: Mysql.DSN 未配置")
	}
	conn := sqlx.NewMysql(cfg.Mysql.DSN)
	cacheConf := cache.CacheConf{
		{
			RedisConf: redis.RedisConf{Host: cfg.IamRedis.Address, Pass: cfg.IamRedis.Password, Type: "node"},
			Weight:    100,
		},
	}
	rdb, err := redis.NewRedis(redis.RedisConf{
		Host: cfg.IamRedis.Address,
		Pass: cfg.IamRedis.Password,
		Type: "node",
	})
	if err != nil {
