...
func (l *LoginLogic) Login(in *iam.LoginRequest) (*iam.TokenPair, error) {
	if in == nil || in.Username == "" || in.Password == "" {
		recordLoginLog(l.svcCtx, 0, in.GetUsername(), in.GetClientIp(), in.GetUserAgent(), "用户名和密码不能为空", false)
	...
	if err != nil {
		if errors.Is(errors.Unwrap(err), sqlx.ErrNotFound) || errors.Is(err, sqlx.ErrNotFound) {
			recordLoginLog(l.svcCtx, 0, in.Username, in.ClientIp, in.UserAgent, "用户名或密码错误", false)
			return nil, toGRPCStatus(errs.New(errs.CodeUnauthorized, "用户名或密码错误"))
		}
		recordLoginLog(l.svcCtx, 0, in.Username, in.ClientIp, in.UserAgent, "查询用户失败", false)
	...
	if bcrypt.CompareHashAndPassword([]byte(user.PasswordHash), []byte(in.Password)) != nil {
		recordLoginLog(l.svcCtx, user.Id, user.Username, in.ClientIp, in.UserAgent, "用户名或密码错误", false)
		return nil, toGRPCStatus(errs.New(errs.CodeUnauthorized, "用户名或密码错误"))
	}
	...
	recordLoginLog(l.svcCtx, user.Id, user.Username, in.ClientIp, in.UserAgent, "登录成功", true)
...
// 提成包级函数供 Login/LoginFeishu 等多个登录渠道复用，不依赖具体 Logic 的 receiver。
func recordLoginLog(svcCtx *svc.ServiceContext, userId uint64, username, clientIP, userAgentStr string, message string, success bool) {
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				logx.Errorf("记录登录日志时发生 panic: %v, userId=%d, username=%s", r, userId, username)
			}
		}()

		if err := svcCtx.Domain.Monitoring.LoginLog.Create(context.Background(), loginLog); err != nil {
			logx.Errorf("记录登录日志失败: userId=%d, username=%s, status=%d, message=%s, error: %v", userId, username, status, message, err)
		}
	}()
}

// createUnreadNoticeNotifications 为新用户创建未读公告通知，提成包级函数供多个登录渠道复用。
func createUnreadNoticeNotifications(svcCtx *svc.ServiceContext, userID uint64) {
	defer func() {
		if r := recover(); r != nil {
			logx.Errorf("创建未读公告通知时发生 panic: %v, userId=%d", r, userID)
		}
	}()

	notices, err := svcCtx.Domain.System.Notice.FindPublishedNotReadByUser(context.Background(), userID)
	if err != nil {
		logx.Errorf("查询未读公告失败: userId=%d, error: %v", userID, err)
		return
	}
	if len(notices) == 0 {
		return
	...
	for _, notice := range notices {
		notifications, _, err := svcCtx.Domain.System.Notification.FindPage(context.Background(), 1, 100, userID, "notice", -1)
		if err == nil {
		...
		if err := svcCtx.Domain.System.Notification.Create(context.Background(), notification); err != nil {
			logx.Errorf("创建公告通知失败: userId=%d, noticeId=%d, error: %v", userID, notice.Id, err)
		}
	}
}
