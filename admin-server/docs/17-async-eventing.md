...
| `BlogArticleAudit`/`BlogArticleAuditUnpublish` 的审计日志写入（`blog/article_audit/*.go`，原 `pkg/audit.RecordAuditLog`） | content-rpc → iam-rpc | **同步 RPC 调用 + 内部包一层 goroutine**（`IamCallback.RecordAuditLog`） | 和原 `pkg/audit.RecordAuditLog` 的既有语义一致——"异步、失败只记日志、不影响主流程"，只是从进程内直调换成跨进程 RPC；之所以不算进"异步 Streams"分类，是因为这是一次性的"写完就走"调用，不需要 Streams 的持久化/消费者组/死信这套机制，普通 RPC 调用外面套一层 `go func(){...}()` + `recover()` 已经足够 |
...
**执行落地时的实际偏差（content-rpc 拆分，`docs/progress.md` 对应条目）**：上表 `PublicBlogAuthorInfo`/`BlogArticleAudit` 两行在拆分前就已经预见到，实际落地时 `pkg/iamcallback`（chat-rpc 拆分时新增，见上一条"执行落地时的实际偏差"）在原有 `FindActiveUserChunk`/`GetUserProfile` 基础上做了两处扩展而不是新增独立契约：① `GetUserProfileResponse` 新增 `signature` 字段（chat-rpc 用不到，留空即可）；② 新增 `RecordAuditLog` 方法（请求体只带 `user_id`/`username`/`audit_type`/`audit_object`/`detail_json`，不透传 IP/UA——原 `pkg/audit.RecordAuditLog` 在这两处调用点本来就是传空 `*http.Request`，IP/UA 恒为空，去掉这两个字段不改变现有行为）。这是"复用已有跨服务契约、按需扩展字段/方法"而不是"每个新跨服务点都新开一个 proto"的模式，和 sdk-rpc 拆分时新增 `SdkCallLogExport`（往已有的 `sdk.proto` 加方法，不新开契约）是同一个做法。
