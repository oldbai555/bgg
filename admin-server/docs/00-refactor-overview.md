...
> 本文档是 `admin-server/docs/` 整个文档集（23 篇 + `progress.md`）的入口和索引。执行者（Cursor / Claude Code）在开始任何一个 Phase 之前，应先完整读一遍本文档，再按需跳转到具体的分篇文档。文档风格：可直接执行的任务说明，不是设计讨论稿（对齐上一轮 DDD-lite 重构任务书的风格，原文档已归档，见 `docs/changelog/archive-backend.md` §15）。

## 0. 这一批文档要解决什么

`admin-server`（go-zero 单体，Go 1.24，模块 `postapocgame/admin-server`）在上一轮 DDD-lite 重构（已完成：目录按 9 域拆分、`internal/domain/{iam,task}` 建立、`ServiceContext` 精简，原任务书已归档，见 `docs/changelog/archive-backend.md` §15）和 Wire 组合根引入（见最近提交 `36fbda9`）之后，架构改造实际只完成了皮层。本轮的目标不是继续修修补补，而是产出一套**详细到可以让 AI 编码工具独立按文档执行**的任务书文档集，覆盖：单体内部加固 → 微服务拆分 → 可观测性/API 文档/CD。**本轮产出的是文档，不是代码**——`00`~`22` 全部 23 篇 + `progress.md` 已随本轮交付完成，`internal/` 下没有任何代码改动；从下一轮开始按 Phase 1 Week 1 顺序实际动代码。
...
- **`admin-server/docs/progress.md` 是本轮重构 Phase 1-3 期间的过程记录（阶段/周次/关键决策），只追加不重写**。仓库根目录曾有一份跨项目生命周期的功能级进度索引 `docs/后端开发进度.md`，2026-07-17 起已按「文档分层与生命周期」规则（`.cursor/rules/00-workflow.mdc`）退场并归档为 `docs/changelog/archive-backend.md`；此后功能行为发生实质变化（不是纯内部重构）按 `AGENTS.md`「完成的定义」的要求，改为在 `docs/changelog/` 补一篇交接记录，不再更新已归档文件。
