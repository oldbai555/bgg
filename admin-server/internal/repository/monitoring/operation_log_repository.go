...
type OperationLogRepository interface {
	FindByID(ctx context.Context, id uint64) (*monitoringmodel.AdminOperationLog, error)
	FindPage(ctx context.Context, page, pageSize int64, userId uint64, username, operationType, operationObject, method, startTime, endTime string) ([]monitoringmodel.AdminOperationLog, int64, error)
	Create(ctx context.Context, log *monitoringmodel.AdminOperationLog) error
	// 批量创建（用于异步写入）
	BatchCreate(ctx context.Context, logs []*monitoringmodel.AdminOperationLog) error
}

type operationLogRepository struct {
	model monitoringmodel.AdminOperationLogModel
	conn  sqlx.SqlConn
}

func NewOperationLogRepository(repo *repository.Repository) OperationLogRepository {
	return &operationLogRepository{model: repo.AdminOperationLogModel, conn: repo.DB}
}

func (r *operationLogRepository) FindByID(ctx context.Context, id uint64) (*monitoringmodel.AdminOperationLog, error) {
	return r.model.FindOne(ctx, id)
}

func (r *operationLogRepository) FindPage(ctx context.Context, page, pageSize int64, userId uint64, username, operationType, operationObject, method, startTime, endTime string) ([]monitoringmodel.AdminOperationLog, int64, error) {
	if page <= 0 {
		page = 1
	}
	if pageSize <= 0 {
	...
	// 查询列表
	var list []monitoringmodel.AdminOperationLog
