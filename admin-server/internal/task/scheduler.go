...
func (s *TaskScheduler) scanAndExecute() {
	...
	taskRepo := repository.NewTaskRepository(s.repo)
...
func (s *TaskScheduler) scanAsyncTasks(ctx context.Context, taskRepo repository.TaskRepository) ([]model.AdminTask, error) {
	// 使用自定义SQL查询异步任务
...
func (s *TaskScheduler) scanScheduledTasks(ctx context.Context, taskRepo repository.TaskRepository) ([]model.AdminTask, error) {
	now := time.Now().Unix()
...
// executeTask 执行任务
func (s *TaskScheduler) executeTask(ctx context.Context, task model.AdminTask) {
	defer func() {
		<-s.semaphore // 释放信号量
		s.wg.Done()
	}()
...
func (s *TaskScheduler) handleTaskError(ctx context.Context, task *model.AdminTask, err error) {
