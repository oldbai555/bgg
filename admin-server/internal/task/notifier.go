...
func (n *TaskNotifier) NotifyTaskStatusChange(ctx context.Context, task *model.AdminTask) {
	defer func() {
		if r := recover(); r != nil {
...
func (n *TaskNotifier) createNotificationRecord(ctx context.Context, task *model.AdminTask) {
	notificationRepo := repository.NewNotificationRepository(n.repo)
...
func (n *TaskNotifier) sendWebSocketMessage(ctx context.Context, task *model.AdminTask) {
	if n.chatHub == nil {
