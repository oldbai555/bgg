...
type TaskExecutor interface {
	...
	Execute(ctx context.Context, task *model.AdminTask, paramsJSON string) (string, error)
}
...
type AsyncTaskBackend interface {
	...
	Submit(ctx context.Context, task *model.AdminTask) (backendTaskID string, err error)

	// Cancel 取消一个任务（如果后端支持）
	Cancel(ctx context.Context, task *model.AdminTask) error

	// SyncStatus 同步任务状态（从后端获取任务状态，更新到 admin_task 表）
	// 说明：对于外部中间件（如 xxl-job），需要定期调用此方法同步状态
	SyncStatus(ctx context.Context, task *model.AdminTask) error
