...
│   ├── logic/<domain>/<module>/    # goctl 生成骨架 + 薄胶水（解析请求 -> 调对应 *RPC -> 映射响应）
│   ├── middleware/        # 手写中间件，全部通过 zrpc client 调用各服务，不再直连数据库
...
└── services/iam/, services/task/, services/sdk/, services/chat/, services/content/
    # Phase 2 拆出的 iam-rpc/task-rpc/sdk-rpc/chat-rpc/content-rpc 五个独立服务，
    # 全部领域代码/repository/model/domain 都已经搬出单体：
    # gateway 的 internal/repository/、internal/model/、internal/domain/ 三个目录已整体删除，
    # 不再直连任何 MySQL。iam-rpc（services/iam/）额外承载 RBAC 权限校验（供
    # PermissionMiddleware/ApiEnabledMiddleware 调用）+ pkg/taskcallback、pkg/iamcallback
    # 两个跨服务回调契约的服务端实现。
```

各服务内部分层一致：`services/<name>/internal/{handler,logic,repository,model,domain,svc}/`，参考实现见 `services/iam/internal/repository/iam/role_permission_repository.go`（squirrel 用法）。
...
- **gateway 的 `ServiceContext` 不持有任何 repository/Domain 字段**：Phase 2 五个服务（iam/task/sdk/chat/content）全部拆分完成后，gateway 只剩 `Redis *redis.Redis`（共享 Redis 直连）+ 每个服务对应的 `XxxRPC xxxclient.Xxx` zrpc client 字段，`internal/logic/<domain>/<module>/` 下的 Logic 一律调 `l.svcCtx.XxxRPC.Method(...)`，不直连任何数据库。各服务自己内部的 `services/<name>/internal/svc/servicecontext.go` 才持有该服务自己的 `Repository`/`Domain *registry.Domain` 聚合（`services/<name>/internal/repository/registry/`），Logic 优先 `l.svcCtx.Domain.IAM.User` 这种调用方式
...
参考实现：`services/iam/internal/repository/iam/role_permission_repository.go`
...
**已知例外（待办，不是可以效仿的先例）**：`services/chat/internal/repository/chat/chat_repository.go` 目前仍有部分方法用静态多行 SQL（参数化、无拼接风险，但不是 squirrel）。新增/修改这个文件时应顺手迁移，而不是继续用旧写法。`services/iam/internal/repository/monitoring/performance_log_repository.go` 已经迁移到 squirrel，不再是例外。
...
- 构造函数 `New<Module>Repository(repo *repository.Repository)`，定义在各服务自己的 `services/<name>/internal/repository/<domain>/`（gateway 已不持有任何 repository）
