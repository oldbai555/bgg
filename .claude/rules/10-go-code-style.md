...
参考实现：`internal/repository/role_permission_repository.go`
...
- 字典 SQL 独立增量文件：`db/migrations/dict_{module}_YYYYMMDD.sql`，用 `ON DUPLICATE KEY UPDATE` 保证幂等，ID 自增，不手动指定；通过 `code` 反查 `type_id`：
  ```sql
