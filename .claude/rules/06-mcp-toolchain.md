...
本项目配置了 **10 个 MCP server**（清单 SSOT：仓库根 `.mcp.json`；上手见 `docs/AI工具链上手.md`）。
...
| 查本地 MySQL / Mongo / Redis 运行数据（调试联调） | **mysql** / **mongodb** / **redis** | 仅在本机服务已启动时使用；MySQL 默认只读 |
...
5. **mysql/mongodb/redis** 不可用 → 不查库，改让用户确认或启动本地服务
