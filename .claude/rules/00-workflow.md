...
| `docker-compose.dev-mixed.yml` | 开发/测试环境部署未合并 `main` 的分支（`ghcr.io` 还没有对应镜像），六服务本地源码 build；MySQL 外部托管账号若只有 1 个库（如 sqlpub 免费版），多个 RPC 服务可以共用同一个库，不强求每服务独立 schema | `script/deploy-dev.sh`（服务器上 git pull + build/up）、`script/deploy-frontend.sh`（本机构建前端 + 上传部署）、`script/ssh-setup.sh`（一键配置目标机 SSH 免密） |
