...
- `admin-server/docs/changelog/`：admin-server 重构 + 部署过程的按日期归档记录（历史/背景，不是规则），`docs/progress.md` 只保留最近条目，索引见 `changelog/README.md`；**2026-07-16 起新增 changelog 条目必须复制 `changelog/TEMPLATE.md`（团队沿用的"上线记录"模板：上线需求/技术优化/线上缺陷 + 变更内容/上线前准备/依赖服务/服务分布情况/故障回滚）填内容，不要自由发挥格式**，实际用例见 `changelog/2026-07-16.md`
...
Phase 2 服务拆分完成后，`admin-server` 从单体变成 gateway + 5 个 RPC 服务，部署方式相应演进出三条平行路径，见 `admin-server/docs/changelog/2026-07-16.md` 的实际部署记录：
