...
- `docs/changelog/`：admin-server + admin-frontend **共用**的开发交接记录（2026-07-17 前叫 `admin-server/docs/changelog/`，只记后端；迁到根目录后前后端共用），**同时是写给下一个没有上下文的新会话的交接文档**（历史/背景 + 强制格式，规则见下方「会话交接文档」一节），`admin-server/docs/progress.md` 只保留最近条目，索引见 `changelog/README.md`；新增条目必须复制 `changelog/TEMPLATE.md` 填内容，不要自由发挥格式，实际用例见 `changelog/2026-07-16.md`
