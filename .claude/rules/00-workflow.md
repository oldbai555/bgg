...
步骤2：评估是否需要数据字典
- 需要：创建增量字典 SQL 文件，路径 `admin-server/db/migrations/dict_{module}_YYYYMMDD.sql`
...
1. **建表 SQL**：`admin-server/db/create_table_<group>.sql`（`id` 自增主键 + `created_at`/`updated_at`/`deleted_at` + 主键/`deleted_at` 索引，业务字段需手动补充）
2. **初始化 SQL**：`admin-server/db/init_<group>.sql`（菜单数据「主菜单 + 新增/编辑/删除按钮」、权限数据「list/create/update/delete」、接口数据「GET/POST/PUT/DELETE」、权限-菜单关联、权限-接口关联，全部主键自增、幂等可重复执行；菜单默认挂在"临时目录"下，路径 `/temp/<group>`，后续在菜单管理里手动挪到正式分类）
...
| Repository 层用 `fmt.Sprintf`/字符串拼接构建 SQL | 有 SQL 注入风险，且与项目风格不一致 | 必须用 `squirrel`（`sq.And/Or/Eq/Like` 等），参考 `internal/repository/role_permission_repository.go` |
...
| 字典SQL插入到 `db/data.sql` | 破坏首次部署与增量更新的边界 | 必须新建独立的 `db/migrations/dict_{module}_YYYYMMDD.sql` |
