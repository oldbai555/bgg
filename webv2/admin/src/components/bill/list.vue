  ...
  <div>
    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'root_category'">
          <span v-if="record.root_category === RootCategory.RootCategoryIncome">收入</span>
          <span v-else-if="record.root_category ===RootCategory.RootCategoryExpenditure">支出</span>
        </template>
...
import {RootCategory,GetBillListReq_Option} from "@/plugin/api/model/lbbill";
...
const getList = async (listOption: Options) => {
  ...
  loading.value = true;
  try {
    const resp = await lbbill.getBillList({
      options: listOption,
    });
...
const handleSearch = (searchValue: string) => {
  opts.opt_list = replaceOps(opts.opt_list, {
    key: GetBillListReq_Option.OptionLikeRemark,
    value: searchValue
  })
  getList(opts)
