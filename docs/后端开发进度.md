...
services/task/, services/sdk/, services/chat/  # Phase 2 拆出的 task-rpc/sdk-rpc/chat-rpc 独立服务
```
...
- **聊天模块（2026-07-12 起拆分为独立服务 chat-rpc，见第 19 节）**：
  ...
  - 跨服务契约：`admin-server/pkg/iamcallback/`（`FindActiveUserChunk`/`GetUserProfile`，单体内实现在 `internal/rpcserver/iamcallback/`）
