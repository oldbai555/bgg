...
- 飞书相关常量（`FeishuProvider`/`FeishuDefaultRoleCode`）收进 `services/iam/internal/consts/consts.go`，不写在 logic 文件里（GGA 审查指出后修复）。
...
**待办（下一次会话前置条件）**：
- 用户已确认飞书角色的部门归属问题（新建的飞书用户 `admin_user.department_id` 目前为 0/未分配，用户管理列表显示"-"），需要补一个默认部门分配策略——下一次会话处理
