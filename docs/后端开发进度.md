...
以上接口的 API 定义、Repository 扩展（`FindPrevArticle`/`FindNextArticle`/`CountPublishedArticles`）、Logic/Handler 实现均尚未完成，落地位置计划为 `internal/logic/public_blog/publicblog{authorinfo,articlestats,articleprev,articlenext}logic.go`，详见第 5 节待办。
