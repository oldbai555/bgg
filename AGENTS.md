...
| `.cursor/rules/00-workflow.mdc` / 本文件第 1.5 节 | Git 分支与 PR 流程（`script/branch_from_main.sh` 系列） | 每次开始新一批改动前 |
...
`main` 是唯一部署源（见 6.0「部署路径」——ghcr 镜像只在 push 到 `main` 时由 CI 构建），日常开发一律从 `main` 切分支、改完开 PR 合回 `main`，不再直接在 `main` 或长期存活的大分支上持续堆提交。

- 分支命名：`<类型>/MMDD_<slug>`，类型对齐 Conventional Commits：`feat`/`fix`/`docs`/`style`/`refactor`/`perf`/`test`/`chore`
- 用 `script/branch_from_main.sh`（或 `script/branch_<type>.sh` 快捷方式，如 `script/branch_feat.sh order-export`）一键完成：拉取 → 有改动自动 stash → 基于 `origin/main` 建分支 → push → `gh pr create`
- 依赖 `gh`（GitHub CLI），需 `gh auth login`；注意个人 shell 配置里可能有其它项目定义的同名 `gh` alias/函数会劫持这个命令，用前先 `type gh` 确认
- squash 与否在 PR 合并时于 GitHub 上选择，脚本不指定合并策略
- PR 可以由 AI 创建，但**合并到 main 默认要用户确认**（见 6 节「何时必须停下来问用户」）
...
| 在 changelog/文档/注释/commit message 里写服务器真实公网 IP | 进 git 历史后很难清除，公开仓库暴露攻击面 | 只写别名（如 `bgg-dev`，对应 `~/.ssh/config` 的 `Host`），真实 IP 不进仓库；域名不算敏感信息 |
...
- PR 合并到 `main`（`script/branch_from_main.sh` 创建 PR 可以自动做，但点击合并默认要用户在 GitHub 上确认，见 1.5 节）
