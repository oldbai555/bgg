...
**关键目录**（后端）：`admin-server/api/admin.api`（唯一 .api 文件）、`internal/{handler,logic}/<domain>/<module>/`（9 业务域）、`internal/repository/<domain>/`、`internal/model/<domain>/`、`internal/domain/iam/`（RBAC 领域服务）、`internal/middleware`、`pkg/errs`、`scripts/generate-*.sh`；`services/task/`（task-rpc，Phase 2 起独立部署单元，见 `admin-server/docs/15-service-boundaries.md`）。维护导航见 [`docs/admin-server-维护导航.md`](docs/admin-server-维护导航.md)。
...
- **目录分层**：handler/logic 按 `internal/<layer>/<domain>/<module>/`；repository/model 按 `internal/<layer>/<domain>/`（package 名 = 域名）；复杂横切逻辑在 `internal/domain/iam/`（RBAC）引入领域服务；任务调度域已拆分成独立服务 `services/task/`（不再是 `internal/domain/task/`），内部同样按 `internal/domain/task/`（调度器/通知器/执行器）分层，只是 scope 收窄到这一个服务自己
