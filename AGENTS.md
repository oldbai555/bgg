...
**关键目录**（后端）：`admin-server/api/admin.api`（唯一 .api 文件）、`internal/{handler,logic}/<domain>/<module>/`（9 业务域）、`internal/repository/<domain>/`、`internal/model/<domain>/`、`internal/domain/iam/`（RBAC 领域服务）、`internal/middleware`、`pkg/errs`、`scripts/generate-*.sh`；`services/task/`（task-rpc）、`services/sdk/`（sdk-rpc）、`services/chat/`（chat-rpc）、`services/content/`（content-rpc，blog+video 域）——Phase 2 起独立部署单元，见 `admin-server/docs/15-service-boundaries.md`；`internal/domain/` 下不再有 `task/`、`sdk/`、`chat/` 子目录，`internal/repository/`、`internal/model/` 下不再有 `blog/`、`video/` 子目录。维护导航见 [`docs/admin-server-维护导航.md`](docs/admin-server-维护导航.md)。
...
- **目录分层**：handler/logic 按 `internal/<layer>/<domain>/<module>/`；repository/model 按 `internal/<layer>/<domain>/`（package 名 = 域名）；复杂横切逻辑在 `internal/domain/iam/`（RBAC）引入领域服务；task/sdk/chat/content 四个域已拆分成独立服务 `services/task/`/`services/sdk/`/`services/chat/`/`services/content/`（不再是 `internal/domain/task/`/`internal/domain/sdk/`/`internal/domain/chat/`/`internal/repository/{blog,video}/`），内部同样按 `services/<name>/internal/domain/<name>/`（task：调度器/通知器/执行器；sdk：`SDKService.SaveApiKeyBindings` 事务；chat：`ChatOnboardingService` + `internal/consumer` 消费 `stream:chat.user.created`；content：`BlogArticleService` 事务）分层，只是 scope 收窄到这一个服务自己；gateway 侧对应的中间件/WS 桥接（`internal/handler/chat/chatwshandler.go`）仍留在网关进程，内部实现改成调 `svcCtx.SdkRPC`/`svcCtx.ChatRPC`/`svcCtx.ContentRPC`；chat-rpc/content-rpc 需要的用户展示信息/存量用户枚举/审计日志写入，通过回调单体内嵌的 `IamCallback` zrpc server（`internal/rpcserver/iamcallback/`，与已有的 `TaskCallback` 同一个"先在单体里实现,等 iam-rpc 拆分后原样搬走"模式）
...
| `mysql` / `mongodb` / `redis` MCP | 可选；需本机服务在跑并配置 `MYSQL_*` 等 env，否则 `make sync-claude-mcp-check` 中可忽略失败 |
