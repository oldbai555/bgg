...
**关键目录**（后端）：`admin-server/api/admin.api`（唯一 .api 文件）、`internal/{handler,logic,repository,model,middleware,consts,types}`、`pkg/errs`、`scripts/generate-*.sh`。
...
3. **用户**执行 `admin-server/scripts/generate-sql.sh -group <group> -name <name>`（AI 不得代替执行）
...
admin-server/admin-frontend 从设计之初就是"工程化"的：新增一个标准 CRUD 业务模块不需要从零手写，步骤 3 的 `generate-sql.sh -group <group> -name <name>` 一条命令会同时生成四块互相匹配、可直接跑通的骨架：
...
- **`.api` 文件**：唯一文件 `admin-server/api/admin.api`；Group 必须 snake_case；禁止路径参数 `:id`，一律 Query/Body 或 `/xxx/detail` 子路径；所有 Query 参数和可选字段必须加 `optional` 标签（`form:"x,optional"` / `json:"x,optional"`），否则 `httpx.Parse` 会报 400
- **SQL 构建强制用 `squirrel`**（别名 `sq`），Repository 层禁止 `fmt.Sprintf`/字符串拼接。参考实现：`internal/repository/role_permission_repository.go`。已知例外/待办：`performance_log_repository.go`、`chat_repository.go` 尚未迁移，不要照抄这两个文件的旧写法
