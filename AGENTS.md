...
Phase 2 服务拆分完成后部署方式演进出三条平行路径，不互相替代：Supervisor 单二进制（老环境，`script/admin.sh supervisor ...`）、`docker-compose.prod.yml`（生产，六服务拉 `ghcr.io` 镜像，`script/admin.sh compose ...`）、`docker-compose.dev-mixed.yml`（开发/测试环境部署未合并 main 的分支，六服务本地 build，`script/deploy-dev.sh` + `script/deploy-frontend.sh` + `script/ssh-setup.sh`）。详见 `.cursor/rules/00-workflow.mdc` 同名小节和 `admin-server/docs/changelog/2026-07-16.md` 的实际部署记录。
