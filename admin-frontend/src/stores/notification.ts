...
export const useNotificationStore = defineStore('notification', () => {
  ...
  function handleChatMessage(data: WSMessage) {
    ...
    // 检查当前是否在聊天页面（ChatList.vue，唯一对应的 admin_menu.path 是 /admin/chatroom/chat，
    // 见 admin-frontend/docs/10-route-namespace-migration.md）。应用全程用 createWebHistory，
    // 不存在 hash 路由场景，不需要额外判断 window.location.hash。
    // 用正则做路径段边界匹配而不是裸 .includes()——admin_menu 里还有 /admin/chatroom/chat-message、
    // /admin/chatroom/chat-group 两个同前缀但语义完全不同的管理页面，裸 includes 会把它们也误判成
    // "在聊天页"，导致真实聊天消息的未读提醒被错误抑制
    const currentPath = window.location.pathname
    const isInChatPage = /(^|\/)admin\/chatroom\/chat(\/|$)/.test(currentPath)
