...
const loadData = async () => {
  loading.value = true
  try {
    const resp = await contentApi.socialInfoList({...query})
...
const handleUpdate = async (row: BlogSocialInfoItem) => {
  try {
    await contentApi.socialInfoUpdate(row as BlogSocialInfoUpdateReq)
...
const handleDelete = (index: number, row: BlogSocialInfoItem) => {
  ElMessageBox.confirm('确定要删除这条社交信息吗？', '确认删除', {type: 'warning'})
    .then(async () => {
      await contentApi.socialInfoDelete({id: row.id})
