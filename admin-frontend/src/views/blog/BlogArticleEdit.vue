...
const loadTagOptions = async () => {
  ...
  const resp = await blogApi.tagOptions({limit: 1000})
...
return
  ...
  try {
    const resp = await blogApi.articleDetail({id: id.value})
...
return
  ...
  try {
    if (isEdit.value) {
      ...
      await blogApi.articleUpdate(req)
      // 根据原状态显示不同的成功提示
      if (articleStatus.value === 1) {
      ...
      await blogApi.articleCreate(req)
...
const handleSubmit = async () => {
  ...
  loading.value = true
  try {
    await blogApi.articleSubmit({id: id.value})
