...
// 状态选项（从字典获取）
const {options: statusOptions} = useDictOptions('blog_friend_link_status', [
  {label: '启用', value: 1},
  {label: '禁用', value: 2}
])
...
const loadData = async () => {
  loading.value = true
  try {
    const resp = await blogFriendLinkList({...query})
...
const handleUpdate = async (row: BlogFriendLinkItem) => {
  try {
    await blogFriendLinkUpdate(row as BlogFriendLinkUpdateReq)
...
const handleAdd = async (row: Record<string, unknown>) => {
  try {
    await blogFriendLinkCreate(row as BlogFriendLinkCreateReq)
...
const handleDelete = (index: number, row: BlogFriendLinkItem) => {
  ElMessageBox.confirm('确定要删除这条友情链接吗？', '确认删除', {type: 'warning'})
    .then(async () => {
      await blogFriendLinkDelete({id: row.id})
