  ...
  <div class="page">
    ...
    <el-card>
      <D2Table
        ...
        :have-edit="true"
        :have-detail="true"
        create-permission="blog_social_info:create"
        update-permission="blog_social_info:update"
        delete-permission="blog_social_info:delete"
...
const list = ref<BlogSocialInfoItem[]>([])
const total = ref(0)
...
const loadData = async () => {
  loading.value = true
  try {
    const resp = await blogSocialInfoList({...query})
...
const handleUpdate = async (row: BlogSocialInfoItem) => {
  try {
    await blogSocialInfoUpdate(row as BlogSocialInfoUpdateReq)
...
const handleAdd = async (row: Record<string, unknown>) => {
  try {
    await blogSocialInfoCreate(row as BlogSocialInfoCreateReq)
...
const handleDelete = (index: number, row: BlogSocialInfoItem) => {
  ElMessageBox.confirm('确定要删除这条社交信息吗？', '确认删除', {type: 'warning'})
    .then(async () => {
      await blogSocialInfoDelete({id: row.id})
