      ...
      >
        ...
        <template #cell="{row, column}">
          ...
          <el-tag v-else-if="column.prop === 'status'" :type="row.status === 2 ? 'success' : 'info'">
            {{ noticeStatusOptions.find(opt => Number(opt.value) === row.status)?.label || (row.status === 2 ? '已发布' : '草稿') }}
          </el-tag>
...
// 公告类型选项
const {options: noticeTypeOptions, getLabel: getNoticeTypeLabel} = useDictOptions(
  'notice_type',
  [
    {label: '普通公告', value: '1'},
    {label: '重要公告', value: '2'},
...
// 公告状态选项
const {options: noticeStatusOptions} = useDictOptions(
  'notice_status',
  [
    {label: '草稿', value: '1'},
    {label: '已发布', value: '2'}
