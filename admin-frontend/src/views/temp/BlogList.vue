  ...
  <div class="page">
    ...
    <el-card>
      <D2Table
        ...
        :have-edit="true"
        :have-detail="true"
        create-permission="blog:create"
        update-permission="blog:update"
        delete-permission="blog:delete"
...
const loadData = async () => {
  ...
  try {
    const req: BlogArticleListReq = {
      ...
      title: query.name || undefined
    }
    const resp = await blogArticleList(req)
...
const handleUpdate = async (row: BlogArticleItem) => {
  try {
    await blogArticleUpdate({
      id: row.id,
      title: row.title
    } as BlogArticleUpdateReq)
...
const handleAdd = async (row: Record<string, unknown>) => {
  try {
    const payload: BlogArticleCreateReq = {
      title: (row.title as string) || '',
      content: '',
      tagIds: []
    }
    await blogArticleCreate(payload)
...
const handleDelete = (_index: number, row: BlogArticleItem) => {
  ElMessageBox.confirm(t('common.confirmDelete'), t('common.confirm'), {type: 'warning'})
    .then(async () => {
      await blogArticleDelete({id: row.id})
