...
const loadData = async () => {
  loading.value = true
  try {
    const resp = await iamApi.roleList({...query})
...
const handleUpdate = async (row: RoleItem) => {
  try {
    await iamApi.roleUpdate(row as RoleUpdateReq)
...
const handleDelete = (index: number, row: RoleItem) => {
  ElMessageBox.confirm(t('common.confirmDelete'), t('common.confirm'), {type: 'warning'})
    .then(async () => {
      await iamApi.roleDelete({id: row.id})
...
const loadPermissions = async () => {
  try {
    const resp = await iamApi.permissionList({page: 1, pageSize: 1000})
    // 将权限列表转换为树形结构（按模块分组）
...
const handleAssignPermissions = async (row: RoleItem) => {
  ...
  // 加载当前角色的权限
  try {
    const resp = await iamApi.rolePermissionList({roleId: row.id})
...
const handleSavePermissions = async () => {
  ...
  try {
    await iamApi.rolePermissionUpdate({roleId: currentRoleId.value, permissionIds})
