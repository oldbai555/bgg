...
const loadData = async () => {
  loading.value = true
  try {
    const resp = await iamApi.permissionList({...query})
...
const handleUpdate = async (row: PermissionItem) => {
  try {
    await iamApi.permissionUpdate(row as PermissionUpdateReq)
...
const handleDelete = (index: number, row: PermissionItem) => {
  ElMessageBox.confirm(t('common.confirmDelete'), t('common.confirm'), {type: 'warning'})
    .then(async () => {
      await iamApi.permissionDelete({id: row.id})
...
const loadMenus = async () => {
  try {
    const resp = await iamApi.menuTree()
...
const loadApis = async () => {
  try {
    const resp = await iamApi.apiList({page: 1, pageSize: 1000})
...
const handleAssignMenus = async (row: PermissionItem) => {
  ...
  // 加载当前权限的菜单
  try {
    const resp = await iamApi.permissionMenuList({permissionId: row.id})
...
const handleSaveMenus = async () => {
  ...
  try {
    ...
    await iamApi.permissionMenuUpdate({permissionId: currentPermissionId.value, menuIds: allKeys})
...
const handleAssignApis = async (row: PermissionItem) => {
  ...
  // 加载当前权限的接口
  try {
    const resp = await iamApi.permissionApiList({permissionId: row.id})
...
const handleSaveApis = async () => {
  ...
  try {
    await iamApi.permissionApiUpdate({permissionId: currentPermissionId.value, apiIds: selectedApiIds.value})
