...
const loadData = async () => {
  loading.value = true
  try {
    const resp = await iamApi.userList({...query})
...
const handleDelete = (index: number, row: UserItem) => {
  ElMessageBox.confirm(t('common.confirmDelete'), t('common.confirm'), {type: 'warning'})
    .then(async () => {
      await iamApi.userDelete({id: row.id})
...
const loadRoles = async () => {
  try {
    const resp = await iamApi.roleList({page: 1, pageSize: 1000})
...
const handleAssignRoles = async (row: UserItem) => {
  ...
  // 加载当前用户的角色
  try {
    const resp = await iamApi.userRoleList({userId: row.id})
...
const handleSaveRoles = async () => {
  ...
  try {
    await iamApi.userRoleUpdate({userId: currentUserId.value, roleIds: selectedRoleIds.value})
