...
const loadData = async () => {
  loading.value = true
  try {
    const resp = await chatApi.chatGroupList({
      page: query.page,
...
const loadUsers = async () => {
  try {
    const resp = await iamApi.userList({page: 1, pageSize: 1000})
    // 过滤掉已经在群组中的用户
...
const handleDelete = (index: number, row: ChatGroupItem) => {
  ElMessageBox.confirm('确定要删除这个群组吗？', '确认删除', {type: 'warning'})
    .then(async () => {
      await chatApi.chatGroupDelete({id: row.id})
...
const handleAddMembers = async () => {
  ...
  memberLoading.value = true
  try {
    await chatApi.chatGroupMemberAdd({
...
const handleRemoveMember = (member: ChatGroupMemberItem) => {
  ElMessageBox.confirm(`确定要将 ${member.nickname || member.username} 移出群组吗？`, '确认移除', {type: 'warning'})
    .then(async () => {
      try {
        await chatApi.chatGroupMemberRemove({
          chatId: currentGroupId.value,
