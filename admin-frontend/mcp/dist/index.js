...
import { z } from 'zod';
import { handleGetComponent, handleGetPatterns, handleListComponents, } from './tools.js';
const server = new McpServer({
    name: 'admin-frontend-ui',
    version: '0.1.0',
});
server.tool('ui_list_components', '列出 admin-frontend 项目内 UI 组件（common/layout/blog），含路径与一句话说明', {
    category: z
...
server.tool('ui_get_component', '获取项目 UI 组件文档：README、关联类型（如 D2Table 的 table.ts）', {
    name: z.string().describe('组件名，如 D2Table、IcpFooter、BlogHeader'),
}, async ({ name }) => ({
    content: [{ type: 'text', text: handleGetComponent(name) }],
}));
server.tool('ui_get_patterns', '获取前端开发约定片段：dict / permission / api / public-pages', {
    topic: z
