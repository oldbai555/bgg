...
export function handleListComponents(category) {
    ...
    const lines = items.map((c) => `- **${c.name}** (${c.category}): ${c.summary}\n  路径: \`${c.path}\``);
    const header = category
        ? `## 组件列表（${category}）\n\n共 ${items.length} 个\n\n`
        : `## 项目 UI 组件列表\n\n共 ${items.length} 个（分类：common / layout / blog）\n\n`;
    return header + lines.join('\n\n');
}
export function handleGetComponent(name) {
    ...
    if (entry.name === 'D2Table' && !entry.readme) {
        parts.push('## 提示', '', 'D2Table 文档见 `src/components/common/README.md`；完整示例参考 `src/views/system/RoleList.vue`。');
    }
    return parts.join('\n');
}
const PATTERNS = {
...
export function handleGetPatterns(topic) {
    if (!topic) {
