...
import (
	...
	"github.com/oldbai555/bgg/service/lbblogserver/impl/conf"
...
func Run(ctx *cli.Context) error {
	...
	// 在gRPC服务器注册我们的服务
	lbblog.RegisterLbblogServer(grpcServer, &service.ServerImpl)
	...
	routine.Go(newCtx, func(ctx context.Context) error {
		...
		log.Infof("listening on %s exited notify", lbblog.ServerName)
		<-signalCh
		log.Warnf("exit: close %s server connect", lbblog.ServerName)
		grpcServer.Stop()
		return nil
	})
