...
import (
	...
	"github.com/oldbai555/bgg/service/lbstoreserver/impl/conf"
...
func Run(ctx *cli.Context) error {
	...
	// 在gRPC服务器注册我们的服务
	lbstore.RegisterLbstoreServer(grpcServer, &service.ServerImpl)
	...
	routine.Go(newCtx, func(ctx context.Context) error {
		register.EndPointToEtcd(ctx, fmt.Sprintf("%s:%d", boundIP, conf.Global.ServerConf.Port), lbstore.ServerName)
		return nil
	})
	...
	routine.Go(newCtx, func(ctx context.Context) error {
		...
		log.Infof("listening on %s exited notify", lbstore.ServerName)
		<-signalCh
		log.Warnf("exit: close %s server connect", lbstore.ServerName)
		grpcServer.Stop()
		return nil
	})
