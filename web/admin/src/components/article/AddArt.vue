        ...
        <a-row :gutter="24">
          ...
          <a-col :span="8">
            <a-form-model-item label="文章分类" prop="category_id">
              <a-select style="width: 200px" v-model="artInfo.category_id" placeholder="请选择分类"
                        @change="cateChange">
...
export default {
  ...
  methods: {
    // 查询文章信息
    async getArtInfo(id) {
      const {data: res} = await this.$http.post(`blog/GetArticle`,
        {
          id: Number(id),
        }
      )
    ...
    upChange(info) {
      ...
      if (info.file.status === 'done') {
        ...
        console.log(info)
        this.artInfo.img = info.file.response.data.url
    ...
    artOk(id) {
      this.$refs.artInfoRef.validate(async (valid) => {
        if (!valid) {
          this.$message.error('参数验证未通过，请按要求录入文章内容')
          return
        }
        if (id === 0) {
          const {data: res} = await this.$http.post('blog/AddArticle', {
            article: this.artInfo
          })
          ...
          const {data: res} = await this.$http.post(`blog/UpdateArticle`, {
            article: this.artInfo
          })
